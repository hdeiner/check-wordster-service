package com.deinersoft.checkwordster.model;
...
public class CheckWordster {
    ...
    private void checkInput() throws CheckWordsterException {
        ...
        if (wordDigits.startsWith("+") || wordDigits.startsWith("-")) {
            if (wordDigits.substring(1, wordDigits.length()).contains("-"))
                throw new CheckWordsterException("Invalid signed number");
            else if (wordDigits.substring(1, wordDigits.length()).contains("+"))
                throw new CheckWordsterException("Invalid signed number");
            else throw new CheckWordsterException("Signed number");
        }
        if (wordDigits.contains(",") && wordDigits.contains(".")) {
            if (!(wordDigits.matches("^[0-9]+(,[0-9]{3})+(\\.[0-9]{0,2})$")))
